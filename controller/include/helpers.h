...
#if PCB_VERSION == 2
// PCBv2: J2 E-stop input is GPIO 21 with R1 10k pull-up to 3V3; an NC loop across J2 reads
// LOW while closed (run) and HIGH when opened OR when a wire breaks - HIGH is the stop state.
#define ESTOP_ACTIVE_STATE 1       // fail-safe: active (stop) when pin is HIGH
#else
#define ESTOP_ACTIVE_STATE 0       // PCBv1: E-stop is active when pin is LOW (normally closed)
#endif
