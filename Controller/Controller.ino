...
void setup(){
   ...
   tmr = xTimerCreate("tmr", pdMS_TO_TICKS(1000), pdFALSE, (void*)0, reinterpret_cast<TimerCallbackFunction_t>(ping));
   xTimerStart(tmr, 0);
}
...
void setupPWMpins() {
    ...
    for(int i=0;i<6;i++)
    {
       ...
       //inputBank.pinMode( lowPins[i], INPUT_PULLUP );
     }
...
void loop()
{    
      //grabs data on the serial for motion changes
