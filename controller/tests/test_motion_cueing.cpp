...
TEST(mca_soft_limit_passthrough_to_knee) {
    const float values[] = {-8.0f, -7.5f, -1.0f, 0.0f, 1.0f, 7.5f, 8.0f};
    for (float value : values) {
        /* The central 80% is exactly linear, including both knee points. */
        ASSERT_EQ(mcaSoftLimit(value, 10.0f, 0.8f), value);
    }
}

TEST(mca_soft_limit_damps_above_knee) {
    const float at_nine = mcaSoftLimit(9.0f, 10.0f, 0.8f);
    const float at_ten = mcaSoftLimit(10.0f, 10.0f, 0.8f);
    const float at_eleven = mcaSoftLimit(11.0f, 10.0f, 0.8f);
    ASSERT_GT(at_nine, 8.0f);
    ASSERT_LT(at_nine, 9.0f);
    ASSERT_GT(at_ten, at_nine);
    ASSERT_LT(at_ten, 10.0f);
    ASSERT_GT(at_eleven, at_ten);
    ASSERT_LT(at_eleven, 10.0f);
    ASSERT_NEAR(at_nine, 8.924234f, 0.00001f);
    /* Equal input increments produce progressively smaller output travel. */
    ASSERT_GT(at_nine - 8.0f, at_ten - at_nine);
    ASSERT_GT(at_ten - at_nine, at_eleven - at_ten);
}

TEST(mca_soft_limit_is_odd_and_bounded) {
    const float limits[] = {0.01f, 1.0f, 10.0f, 100.0f};
    for (float limit : limits) {
        float previous = 0.0f;
        for (int i = 0; i <= 500; ++i) {
            const float value = limit * (float)i / 100.0f;
            const float positive = mcaSoftLimit(value, limit, 0.8f);
            const float negative = mcaSoftLimit(-value, limit, 0.8f);
            /* Inclusive at the rail: float tanh may round to exactly one. */
            ASSERT(positive >= previous && positive <= limit);
            ASSERT(negative >= -limit && negative <= 0.0f);
...
TEST(mca_soft_limit_nonpositive_limit) {
    const float values[] = {-1000.0f, -1.0f, 0.0f, 1.0f, 1000.0f};
    for (float value : values) {
        ASSERT_EQ(mcaSoftLimit(value, 0.0f, 0.8f), 0.0f);
        ASSERT_EQ(mcaSoftLimit(value, -1.0f, 0.8f), 0.0f);
    }
}

TEST(mca_soft_limit_clamps_knee) {
    const float values[] = {-100.0f, -9.8f, -2.0f, 0.0f, 2.0f, 9.8f, 100.0f};
    for (float value : values) {
        ASSERT_EQ(mcaSoftLimit(value, 10.0f, -0.5f), mcaSoftLimit(value, 10.0f, 0.0f));
        ASSERT_EQ(mcaSoftLimit(value, 10.0f, 1.0f), mcaSoftLimit(value, 10.0f, 0.95f));
        ASSERT_EQ(mcaSoftLimit(value, 10.0f, 2.0f), mcaSoftLimit(value, 10.0f, 0.95f));
    }
    ASSERT_LT(mcaSoftLimit(2.0f, 10.0f, -0.5f), 2.0f);
    ASSERT_EQ(mcaSoftLimit(9.0f, 10.0f, 2.0f), 9.0f);
}

TEST(mca_soft_limit_knee_continuity) {
    const float epsilon = 0.001f;
    const float below = mcaSoftLimit(8.0f - epsilon, 10.0f, 0.8f);
    const float at = mcaSoftLimit(8.0f, 10.0f, 0.8f);
    const float above = mcaSoftLimit(8.0f + epsilon, 10.0f, 0.8f);
    ASSERT_LT(below, at);
    ASSERT_GT(above, at);
    /* No position jump or abrupt slope change as damping begins. */
    ASSERT_NEAR((at - below) / epsilon, 1.0f, 0.002f);
    ASSERT_NEAR((above - at) / epsilon, 1.0f, 0.002f);
}
...
void register_motion_cueing_tests(void) {
    ...
    RUN_TEST(mca_soft_limit_passthrough_to_knee);
    RUN_TEST(mca_soft_limit_damps_above_knee);
    RUN_TEST(mca_soft_limit_is_odd_and_bounded);
    RUN_TEST(mca_soft_limit_nonpositive_limit);
    RUN_TEST(mca_soft_limit_clamps_knee);
    RUN_TEST(mca_soft_limit_knee_continuity);
}
